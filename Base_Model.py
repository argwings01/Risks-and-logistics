...
obj_eqn = xp.Sum(z[j, t] * TotalSetupCost[j] +
       y[j, t] * OperatingCost[j] + xp.Sum(
      xp.Sum(CostSupplierWarehouse[(j,k)] * M[k, j, t, q] for k in Supplier) +
      xp.Sum(CostWarehouseCustomer[(j,i)] * demand_matrices[t+1][q-1, i-1] * x[i, j, t] for i in Customer)  for q in Products)
      for j in Groups for t in Period)
...
prob.addConstraint(
    y[j,t] * supplier_cap_matrix[k,q]>=M[k, j, t, q]>= 0
    for k in Supplier for j in Groups for t in Period for q in Products)


prob.addConstraint(
    xp.Sum(demand_matrices[t+1][q, i] * x[i,j,t] for i in Customer)
    <= xp.Sum(M[k,j,t,q] for k in Supplier)
    for j in Groups for q in Products for t in Period)


prob.addConstraint(
    xp.Sum(demand_matrices[t+1][q, i] * x[i,j,t] for i in Customer for q in Products)<= 
    xp.Sum(M[k, j, t, q] for k in Supplier for q in Products) <= TotalCapacity[j] * y[j, t]
    for j in Groups for t in Period)
...
#             for k in Supplier: 
#                 for q in Products:
#                     if prob.getSolution(M[k,j,t,q])>0:
#                         print("M(",k,",",j,",",t,",",q,") is: ", prob.getSolution(M[k,j,t,q]))
                        
# for i in Customer:
#     for j in Groups:
#         for t in Period:
#             if prob.getSolution(x[i,j,t])>0:
#                 print("x(",i,",",j,",",t,") is: ", prob.getSolution(x[i,j,t]))
...
#                 "Warehouse": j,
#                 "Period": t,
#                 "Open": prob.getSolution(y[j,t])
#             })

# df_y = pd.DataFrame(rows)
# df_y.to_excel("Results_y_{j,t}.xlsx", index=False)

# rows = []
# for i in Customer:
#     for j in Groups:
#         for t in Period:
#             if prob.getSolution(x[i,j,t]) > 0.5:
#                 rows.append({
#                     "Customer": i,
#                     "Warehouse": j,
#                     "Period": t,
#                     "Open": prob.getSolution(x[i,j,t])
#                     })

# df_x = pd.DataFrame(rows)
# df_x.to_excel("Results_x_{i,j,t}.xlsx", index=False)

# print(prob.attributes.objval)
# rows = []
# for j in Groups:
#     for t in Period:
#         if prob.getSolution(y[j,t]) > 0.5:
...
#                             "Supplier": k,
#                             "Products": q,
#                             "Amount": prob.getSolution(M[k,j,t,q])
#                             })

# df_x = pd.DataFrame(rows)
# df_x.to_excel("Results_M_{k,j,t,q}.xlsx", index=False)
